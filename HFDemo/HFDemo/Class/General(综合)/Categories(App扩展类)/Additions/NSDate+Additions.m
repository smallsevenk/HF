...
- (BOOL)isSameDayAsDate:(NSDate * _Nonnull) aDate{
...
- (BOOL)isInWithMinDate:(NSDate * _Nonnull) aDate maxDate:(NSDate * _Nonnull) bDate{
    if ([self isLaterThanOrEqualDate:aDate] && [self isEarlierThanOrEqualDate:bDate]) {
        return true;
    }
    return false;
}
...
- (NSInteger)yearsAfterDate:(NSDate * _Nonnull) aDate {
...
- (NSInteger)yearsBeforeDate:(NSDate * _Nonnull) aDate {
...
- (NSString*)cnDay
{
    ...
    else if([self isEqualToDateIgnoringTime:[NSDate dateWithDaysFromNow:2]]){
        return @"后天";
    }
