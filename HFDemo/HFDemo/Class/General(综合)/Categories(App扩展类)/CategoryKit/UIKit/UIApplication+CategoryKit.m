...
//  CategoryKit
//
//  Created by smallsevenk on 13/4/4.
...
#define kNetworkIndicatorDelay_ (1/30.0)

@interface _XUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _XUIApplicationNetworkIndicatorInfo
@end
...
- (BOOL)isPirated {
    ...
    if (![self _x_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _x_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_x_fileExistInMainBundle:(NSString *)name {
...
XSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _XUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _XUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_XUIApplicationNetworkIndicatorInfo new];
            ...
            info.timer = [NSTimer timerWithTimeInterval:kNetworkIndicatorDelay_ target:self selector:@selector(_delaySetActivity:) userInfo:@(info.count > 0) repeats:NO];
