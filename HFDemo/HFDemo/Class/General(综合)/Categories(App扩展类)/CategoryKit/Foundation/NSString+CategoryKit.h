...
#define kCurrencySymbolString(money)    [NSString currencySymbolStringWithMoney:CategoryValue(money)]
///不带货币符号(eg. 33,333,33.00)
#define kCurrencyNoSymbolString(money)  [NSString currencyNoSymbolStringWithMoney:CategoryValue(money)]


@interface NSString (CategoryKit)

+ (instancetype)stringWithNilFormat:(nullable NSString *)format, ... NS_FORMAT_FUNCTION(1,2);

///移动 联通 电信 小灵通 (此方法不全...需要维护)
- (nonnull NSString *)mobileServiceName;

///科学计数
- (NSString *)numberFormatter:(NSString *)num;

///金额  四舍五入  position 几位小数
+ (NSString *)roundingFormatWithFloat:(double)price afterPoint:(NSInteger)position;


///带货币符号￥(eg.  ￥3,333,333.00)  支持NSString 和 NSNumber
+ (NSString *)currencySymbolStringWithMoney:(id)money;
///不带货币符号(eg. 33,333,33.00) 支持NSString 和 NSNumber
+ (NSString *)currencyNoSymbolStringWithMoney:(id)money;


#pragma mark stringBy操作
///追加一个string
...
///1KB 2G eg. byte:1000000 divisor:1024
...
- (nullable NSString *)md5String_16;
